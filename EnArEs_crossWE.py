...
def train_epoch(model, train_data, loss_function, optimizer, pretrainedWE, word2id, i, all_losses):
    model.train()
    ...
    for sentwords, dgr in train_data:
        ...
        sent = dataLoaderRegresser.prepare_sequence_pretrainedVec(sentwords, pretrainedWE, word2id)
...
def evaluate(model, data, loss_function, pretrainedWE, word2id, all_losses_dev, name='dev'):
    model.eval()
    avg_loss = 0.0
    truth_res = []
    pred_res = []
...
def trainArEs(model, loss_Function, optimizer, train_data, dev_data, pretrainedWE, word2id, pretrainedWE_en, word2id_en, emotion, seed, testfile, predpath, EPOCH=50):
    #best_dev_acc = 0.0
    ...
    for i in range(EPOCH):
        ...
        if dev_avgloss < best_dev_avgloss:
            ...
            all_losses_test = []
            test_avgloss, test_pred_res, _ = evaluate(model, test_data, loss_Function, pretrainedWE_en, word2id_en, all_losses_test, 'test')
            ...
            if no_drop >= 10:
                #exit()
                break
    return model, all_losses, all_losses_dev, best_dev_avgloss
...
if __name__ == '__main__':
    ...
    emb_pathEn = 'pretrainedWE/wiki.multi.en.vec'
    embeddingsEn, id2wordEn, word2idEn = dataLoaderRegresser.load_vec(emb_pathEn)

    emb_pathEs = 'pretrainedWE/wiki.multi.es.vec'
    embeddingsEs, id2wordEs, word2idEs = dataLoaderRegresser.load_vec(emb_pathEs)

    emb_pathAr = 'pretrainedWE/wiki.multi.ar.vec'
    embeddingsAr, id2wordAr, word2idAr = dataLoaderRegresser.load_vec(emb_pathAr)

    for emotion in emotionlist:
        print(emotion)

        trainfile = 'data/EI-reg-En-train/EI-reg-En-' + emotion + '-train.txt'
        trainAr = 'data/2018-EI-reg-Ar-train/2018-EI-reg-Ar-' + emotion + '-train.txt'
        trainEs = 'data/2018-EI-reg-Es-train/2018-EI-reg-Es-' + emotion + '-train.txt'

        devfile = 'data/2018-EI-reg-En-dev/2018-EI-reg-En-' + emotion + '-dev.txt'
        devAr = 'data/2018-EI-reg-Ar-dev/2018-EI-reg-Ar-' + emotion + '-dev.txt'
        devEs = 'data/2018-EI-reg-Es-dev/2018-EI-reg-Es-' + emotion + '-dev.txt'
        ...
        testAr = 'data/SemEval2018-Task1-AIT-Test-gold/EI-reg/2018-EI-reg-Ar-'+emotion+'-test-gold.txt'
        testEs = 'data/SemEval2018-Task1-AIT-Test-gold/EI-reg/2018-EI-reg-Es-'+emotion+'-test-gold.txt'


        train_data, dev_data, test_data, word_to_ix = dataLoaderRegresser.loadData(trainfile, devfile, testfile)
        print('Arabic')
        train_dataAr, dev_dataAr, test_dataAr, word_to_ixAr, char_to_ixAr = dataLoaderRegresser.loadDataArEs(trainAr, devAr, testAr)

        trainDevTestAr = train_dataAr + dev_dataAr + test_dataAr
        random.shuffle(trainDevTestAr)
        train_dataAr = trainDevTestAr[:int(len(trainDevTestAr)*0.9)]
        dev_dataAr = trainDevTestAr[int(len(trainDevTestAr)*0.9):]

        print('Spanish')
        train_dataEs, dev_dataEs, test_dataEs, word_to_ixEs, char_to_ixEs = dataLoaderRegresser.loadDataArEs(trainEs, devEs, testEs)
        trainDevTestEs = train_dataEs + dev_dataEs + test_dataEs
        random.shuffle(trainDevTestEs)
        train_dataEs = trainDevTestEs[:int(len(trainDevTestEs) * 0.9)]
        dev_dataEs = trainDevTestEs[int(len(trainDevTestEs) * 0.9):]
        ...
        for SEED in seedlist:
            ...
            #----------------------------------------------
            predpathAr = 'predictions/ArEsEn/Ar/'
            #predpathEs = 'predictions/ArEsEn/Es/'
            if not os.path.exists(predpathAr):
                os.makedirs(predpathAr)
            #if not os.path.exists(predpathEs):
            #    os.makedirs(predpathEs)

            # Train on Arabic or Spanish
            model, all_losses, all_losses_dev, best_dev_avgloss = trainArEs(model, loss_Function, optimizer,train_dataAr, dev_dataAr, embeddingsAr,word2idAr, embeddingsEn, word2idEn, emotion, SEED, testfile, predpathAr, EPOCH=EPOCH)
            #model, all_losses, all_losses_dev, best_dev_avgloss = trainArEs(model, loss_Function, optimizer, train_dataEs, dev_dataEs, embeddingsEs, word2idEs, embeddingsEn, word2idEn, emotion, SEED, testfile, predpathEs, EPOCH=EPOCH)

            plotpath = 'plots/ArEsEn/Ar/'
            #plotpath = 'plots/ArEsEn/Es/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)

            plotLosses(all_losses, all_losses_dev, plotpath + emotion + str(SEED) + '.pdf')
            #-----------------------------------------------

            # Train on English

            predpath = 'predictions/ArEsEn/En_Ar/'
            if not os.path.exists(predpath):
                os.makedirs(predpath)

            test_pred_res, all_losses, all_losses_dev = train(model, loss_Function, optimizer, train_data, dev_data, test_data, embeddingsEn, word2idEn, emotion, SEED, testfile, predpath, best_dev_avgloss, EPOCH=EPOCH)


            #test_pred_res, all_losses, all_losses_dev = train(model, loss_Function, optimizer, train_data, dev_data, test_data, word_to_ix, EPOCH=EPOCH)

            #plot training losses
            #plotLosses(all_losses, 'plots/BiLSTMdropout0.1layer2/en-'+emotion+str(SEED)+'-train.pdf')

            #plot dev losses
            #plotLosses(all_losses_dev, 'plots/BiLSTMdropout0.1layer2/en-'+emotion+str(SEED)+'-dev.pdf')
            plotpath = 'plots/ArEsEn/En_Ar/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)
