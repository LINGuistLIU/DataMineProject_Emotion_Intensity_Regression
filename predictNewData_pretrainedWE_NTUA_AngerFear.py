...
class LSTMRegresser(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, n_output, bidirectional=True, dropout=0.1, num_layers=2):
    ...
    def forward(self, sentence):
        ...
        #print('->', sentence)
        #print('->', sentence.type())
        x = sentence.view(len(sentence), 1, -1)
        #print('-> x: ', x)
...
def train(model, loss_Function, optimizer, train_data, dev_data, test_data, word_to_ix, emotion, seed, testfile, predpath, new_data_list, newDatafileList, predpathNew, id_predNewfilepath_dict, best_char_dev_avgloss=1.0, EPOCH=100):
    #best_dev_acc = 0.0
    ...
    for i in range(EPOCH):
        ...
        if dev_avgloss < best_dev_avgloss:
            ...
            inew = 0
            for new_data, newfile in zip(new_data_list, newDatafileList):
                all_losses_new = []
                new_avgloss, new_pred_res, _ = evaluate(model, new_data, loss_Function, word_to_ix, all_losses_new, 'new')
                predNewpathnow = predpathNew + id_predNewfilepath_dict[inew]+emotion+str(seed)+'/'
                print('->Predicting', predNewpathnow, '...')
                if not os.path.exists(predNewpathnow):
                    os.makedirs(predNewpathnow)
                predNewfile = predNewpathnow + 'epoch'+str(i)+'loss'+str(round(best_dev_avgloss, 4))+ '.txt'
                dataLoaderRegresser.outputNewPred(newfile, predNewfile, new_pred_res)
                inew += 1
    ...
    for new_data in new_data_list:
        all_losses_new = []
        new_avgloss, new_pred_res, _ = evaluate(model, new_data, loss_Function, word_to_ix, all_losses_new, 'new')
        new_pred_res_list.append(new_pred_res)

    return test_pred_res, new_pred_res_list, all_losses, all_losses_dev
...
if __name__ == '__main__':
    ...
    for emotion in emotionlist:
        ...
        ## BP data
        #newBPpath = 'newDataProcessed/BP/'+emotion+'/'
        #newBPfile1 = newBPpath + 'bus_' + emotion + '.txt'
        #newBPfile2 = newBPpath + 'journ_' + emotion + '.txt'
        #newBPfile3 = newBPpath + 'photo_' + emotion + '.txt'
        #newBPfile4 = newBPpath + 'student_' + emotion + '.txt'
        #newBPfile5 = newBPpath + 'teacher_' + emotion + '.txt'

        ## Rishitha data
        newRishithapath = 'newDataProcessed/Rishitha/'+emotion+'/'
        newRfile1 = newRishithapath + 'bus_' + emotion + '.txt'
        newRfile2 = newRishithapath + 'journ_' + emotion + '.txt'
        newRfile3 = newRishithapath + 'photo_' + emotion + '.txt'
        newRfile4 = newRishithapath + 'student_' + emotion + '.txt'
        newRfile5 = newRishithapath + 'teacher_' + emotion + '.txt'

        ## newData1
        newDatapath = 'newDataProcessed/newData1/' + emotion + '/'
        newDatafile1 = newDatapath + 'bus_' + emotion + '.txt'
        newDatafile2 = newDatapath + 'journ_' + emotion + '.txt'
        newDatafile3 = newDatapath + 'photo_' + emotion + '.txt'
        newDatafile4 = newDatapath + 'student_' + emotion + '.txt'
        newDatafile5 = newDatapath + 'teacher_' + emotion + '.txt'

        ## Rishitha_topic data
        newRtopicpath = 'newDataProcessed/Rishitha_topic/'
        newRTfile = newRtopicpath + emotion + '.txt'

        #newDatafileList = [newBPfile1, newBPfile2, newBPfile3, newBPfile4, newBPfile5,
        #                   newRfile1, newRfile2, newRfile3, newRfile4, newRfile5,
        #                   newDatafile1, newDatafile2, newDatafile3, newDatafile4, newDatafile5,
        #                   newRTfile]

        newDatafileList = [newRfile1, newRfile2, newRfile3, newRfile4, newRfile5,
                           newDatafile1, newDatafile2, newDatafile3, newDatafile4, newDatafile5,
                           newRTfile]

        #id_predNewfilepath_dict = {0:'BP/bus/', 1:'BP/journ/', 2:'BP/photo/', 3:'BP/student/', 4:'BP/teacher/',
        #                           5:'Rishitha/bus/', 6:'Rishitha/journ/', 7:'Rishitha/photo/', 8:'Rishitha/student/', 9:'Rishitha/teacher/',
        #                           10:'newDataPrevious/bus/', 11:'newDataPrevious/journ/', 12:'newDataPrevious/photo/', 13:'newDataPrevious/student/', 14:'newDataPrevious/teacher/',
        #                           15:'Rishitha_topic/'}
        id_predNewfilepath_dict = {0:'Rishitha/bus/', 1:'Rishitha/journ/', 2:'Rishitha/photo/', 3:'Rishitha/student/', 4:'Rishitha/teacher/',
                                   5:'newDataPrevious/bus/', 6:'newDataPrevious/journ/', 7:'newDataPrevious/photo/', 8:'newDataPrevious/student/', 9:'newDataPrevious/teacher/',
                                   10:'Rishitha_topic/'}

        train_data, dev_data, test_data, new_data_list, word_to_ix = dataLoaderRegresser.loadDataSTandNew(trainfile, devfile, testfile, newDatafileList)
        for SEED in seedlist:
            ...
            model = LSTMRegresser(embedding_dim=EMBEDDING_DIM, hidden_dim=HIDDEN_DIM, n_output=1)
            ...
            predpath = 'predictions/pretrainedWE_NTUA/100Epoch20EarlyStop/'
            predpathNew = 'predictions/pretrainedWE_NTUA/newData/'
            if not os.path.exists(predpath):
                os.makedirs(predpath)

            best_char_dev_avgloss = 1.0
            test_pred_res, new_pred_res_list, all_losses, all_losses_dev = train(model, loss_Function, optimizer, train_data, dev_data, test_data, word_to_ix, emotion, SEED, testfile, predpath, new_data_list, newDatafileList, predpathNew, id_predNewfilepath_dict, best_char_dev_avgloss=best_char_dev_avgloss, EPOCH=EPOCH)

            plotpath = 'plots/pretrainedWE_NTUA/100Epoch20EarlyStop/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)
            plotLosses(all_losses, all_losses_dev, plotpath + 'en-' + emotion + str(SEED) + '.pdf')
            ...
            inew = 0
            print('->len(new_pred_res_list %d, len(newDatafileList %d)' % (len(new_pred_res_list), len(newDatafileList)))
            for new_pred_res, newfile in zip(new_pred_res_list, newDatafileList):
                predNewfile = predpathNew +id_predNewfilepath_dict[inew]+ 'en-' + emotion + str(SEED) + '.txt'
                print('->Writing out', predNewfile, '...')
                dataLoaderRegresser.outputNewPred(newfile, predNewfile, new_pred_res)
                inew += 1
