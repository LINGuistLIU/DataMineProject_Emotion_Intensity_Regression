...
if __name__ == '__main__':
    ...
    for emotion in emotionlist:
        ...
        train_dataAr, dev_dataAr, test_dataAr, word_to_ixAr, char_to_ixAr = dataLoaderRegresser.loadDataChar(trainfileAr, devfileAr, testfileAr)
        ...
        testfileEs = 'translated/Es_' + emotion + '_test.txt'
        train_dataEs, dev_dataEs, test_dataEs, word_to_ixEs, char_to_ixEs = dataLoaderRegresser.loadDataChar(trainfileEs, devfileEs, testfileEs)
        train_dev_dataEs = train_dataEs + dev_dataEs + test_dataEs
        random.shuffle(train_dev_dataEs)
        train_dataEs = train_dev_dataEs[:int(len(train_dev_dataEs) * 0.9)]
        dev_dataEs = train_dev_dataEs[int(len(train_dev_dataEs) * 0.9):]


        NEWword2id = {}
        for i, v in word_to_ix.items():
            NEWword2id[i] = v
        for i, v in word_to_ixEs.items():
            if i not in NEWword2id:
                NEWword2id[i] = len(NEWword2id)
        for i, v in word_to_ixAr.items():
            if i not in NEWword2id:
                NEWword2id[i] = len(NEWword2id)
        word_to_ix = NEWword2id
        ...
        for SEED in seedlist:
            ...
            #-------------------------------------------------------------
            """
            predpath = 'predictions/EnArEs_translateNTUA/Es/'
            if not os.path.exists(predpath):
                os.makedirs(predpath)

            best_char_dev_avgloss = 1.0
            test_pred_res, all_losses, all_losses_dev = train(model, loss_Function, optimizer, train_dataEs, dev_dataEs, test_data, word_to_ix, emotion, SEED, testfile, predpath, best_char_dev_avgloss=best_char_dev_avgloss, EPOCH=EPOCH)

            plotpath = 'plots/EnArEs_translateNTUA/Es/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)
            plotLosses(all_losses, all_losses_dev, plotpath + 'en-' + emotion + str(SEED) + '.pdf')

            """
            predpath = 'predictions/EnArEs_translateNTUA/Ar/'
            if not os.path.exists(predpath):
                os.makedirs(predpath)

            best_char_dev_avgloss = 1.0
            test_pred_res, all_losses, all_losses_dev = train(model, loss_Function, optimizer, train_dataAr, dev_dataAr,test_data, word_to_ix, emotion, SEED, testfile, predpath,best_char_dev_avgloss=best_char_dev_avgloss, EPOCH=EPOCH)

            plotpath = 'plots/EnArEs_translateNTUA/Ar/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)
            plotLosses(all_losses, all_losses_dev, plotpath + 'en-' + emotion + str(SEED) + '.pdf')
            ...
            predpath = 'predictions/EnArEs_translateNTUA/En_Ar/'
            if not os.path.exists(predpath):
                os.makedirs(predpath)
            ...
            plotpath = 'plots/EnArEs_translateNTUA/En_Ar/'
            if not os.path.exists(plotpath):
                os.makedirs(plotpath)
            plotLosses(all_losses, all_losses_dev, plotpath + 'en-' + emotion + str(SEED) + '.pdf')
